<template>
	<div class="main-content" :style='{"padding":"30px","margin":"0"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">线路名称</label>
						<el-input v-model="searchForm.xianlumingcheng" placeholder="线路名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">起点</label>
						<el-input v-model="searchForm.qidian" placeholder="起点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">终点</label>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('xianlubaoming','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('xianlubaoming','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu15" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						删除
					</el-button>


					<!-- <el-button class="btn18" v-if="isAuth('xianlubaoming','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						审核
					</el-button> -->
					<el-button class="btn18" v-if="isAuth('xianlubaoming','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('xianlubaoming','线路金额')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='false' prop="xianlutupian" width="200" label="线路图片">
						<template slot-scope="scope">
							<div v-if="scope.row.xianlutupian">
								<img v-if="scope.row.xianlutupian.substring(0,4)=='http'" :src="scope.row.xianlutupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.xianlutupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="订单状态">
						<template slot-scope="scope">
							<span style="margin-right:10px">{{scope.row.ispay}}</span>
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('xianlubaoming','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' prop="sfsh" label="参团人数">
						<template slot-scope="scope">
							{{scope.row.cantuanrenshu}}
						</template>
					</el-table-column>
					
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xianlubaoming','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('xianlubaoming','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-bianji" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								修改
							</el-button>
							<el-button class="btn18"  type="success" @click="shBatchDialog(scope.row.id)" v-if="scope.row.ispay == '申请退款'">
								<span class="icon iconfont icon-shenhe19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								审核
							</el-button>
							...
							<el-button class="del" v-if="isAuth('xianlubaoming','删除') " type="primary" @click="deleteHandler(scope.row.id )">
			...
			<el-form ref="form" :model="form" label-width="80px">
				<el-form-item label="审核状态">
					<el-select v-model="shBatchForm.sfsh" placeholder="审核状态">
						<el-option label="同意" value="已退款"></el-option>
						<el-option label="不同意" value="已支付"></el-option>
					</el-select>
				</el-form-item>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="lvyoujiageChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			payHandler(row){
				...
				this.$storage.set('paytable','xianlubaoming');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var lvyoujiageChart1 = echarts.init(document.getElementById("lvyoujiageChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                lvyoujiageChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    lvyoujiageChart1.resize();
                };
            }
        });
      })
    ...
    init () {
		...
		this.isPayOptions = "已支付,未支付,申请退款".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    //批量审核
    shBatchHandler(){
      this.$confirm(`是否${this.ids.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$http({
          url: "xianlubaoming/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
	...
	xianlubaomingstatusChange(e,row){
		if(row.status==0){
