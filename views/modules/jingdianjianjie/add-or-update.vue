	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="员工名称" prop="yuangongmingcheng">
					<el-select :disabled="ro.yuangongmingcheng" v-model="ruleForm.yuangongmingcheng" placeholder="请选择员工名称" >
						<el-option
							v-for="(item,index) in yuangongmingchengOptions"
							v-bind:key="item.id"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="员工名称" prop="yuangongmingcheng">
					<el-input v-model="ruleForm.yuangongmingcheng"
						placeholder="员工名称" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="景点等级" prop="jingdiandengji">
...
export default {
		...
		init(id,type) {
            ...
            this.$http({
				...
				if (data && data.code === 0) {
					...
					console.log(this.jingdianfenleiOptions)
				} else {
					this.$message.error(data.msg);
				}
            });
			...
			this.$http({
				...
				if (data && data.code === 0) {
					this.yuangongmingchengOptions = data.data.list
					console.log(this.yuangongmingchengOptions)
				} else {
					this.yuangongmingchengOptions = []
				}
			});
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.jingdianjieshao = this.ruleForm.jingdianjieshao.replace(reg,'../../../springboot2e667147/upload');
        this.ruleForm.zhuyishixiang = this.ruleForm.zhuyishixiang.replace(reg,'../../../springboot2e667147/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jingdianjianjie/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jingdianjianjieCrossAddOrUpdateFlag = false;
											this.parent.search();
